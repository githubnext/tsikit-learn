...
export class ComplementNB {
	...
	fit(X: Float64Array[], y: Int32Array): this {
		...
		for (let i = 0; i < n; i++) {
			...
			this.classCount_[ci] = (this.classCount_[ci] ?? 0) + 1;
			for (let j = 0; j < nFeatures; j++) {
				this.featureCount_[ci]![j] = (this.featureCount_[ci]![j] ?? 0) + (X[i]?.[j] ?? 0);
			}
		}
		...
		for (let ci = 0; ci < nClasses; ci++) {
			for (let j = 0; j < nFeatures; j++) totalFeatureCount[j] = (totalFeatureCount[j] ?? 0) + (this.featureCount_[ci]![j] ?? 0);
		}
	...
	fit(X: Float64Array[], y: Int32Array): this {
		...
		for (let i = 0; i < n; i++) {
			...
			for (let j = 0; j < nFeatures; j++) {
				...
				this.featureCount_[ci]![j] = (this.featureCount_[ci]![j] ?? 0) + val;
			}
		}
