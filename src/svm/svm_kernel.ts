...
export function kernelMatrix(
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    for (let j = 0; j < m; j++) {
      ...
      switch (kernel) {
        case "linear":
          row[j] = linearKernel(x, y);
          break;
        case "poly":
          row[j] = polyKernel(x, y, degree, gamma, coef0);
          break;
        case "rbf":
          row[j] = rbfKernel(x, y, gamma);
          break;
        case "sigmoid":
          row[j] = sigmoidKernel(x, y, gamma, coef0);
          break;
        default:
          row[j] = rbfKernel(x, y, gamma);
          break;
