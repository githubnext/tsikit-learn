...
export type SparseCodingAlgorithm =
  | "lasso_lars"
  | "lasso_cd"
  | "lars"
  | "omp"
  ...
  transform(X: Float64Array[]): Float64Array[] {
    ...
    for (let i = 0; i < nSamples; i++) {
      ...
      if (this.transform_algorithm === "threshold") {
        ...
        const nNonzero =
          this.transform_n_nonzero_coefs ?? Math.ceil(x.length / 10);
        ...
        for (let step = 0; step < nNonzero; step++) {
          ...
          let proj = 0;
          for (let j = 0; j < residual.length; j++)
            proj += (residual[j] ?? 0) * (atom[j] ?? 0);
          const alpha = proj / (atomNorm || 1);
          code[bestAtom] = alpha;
          for (let j = 0; j < residual.length; j++)
            residual[j]! -= alpha * (atom[j] ?? 0);
        }
      } else {
