...
export class GaussianMixture {
  ...
  private _eStep(
    ...
    const resp: Float64Array[] = Array.from(
      { length: n },
      () => new Float64Array(k),
    );
    for (let i = 0; i < n; i++) {
      ...
      for (let c = 0; c < k; c++) {
        const logP =
          Math.log(weights[c] ?? 1 / k) +
          this._logNormalPdf(
            X[i] as Float64Array,
            means[c] as Float64Array,
            variances[c] ?? 1,
          );
        r[c] = Math.exp(logP);
        sumR += r[c] ?? 0;
  ...
  fit(X: Float64Array[]): this {
    ...
    for (let iter = 0; iter < this.maxIter; iter++) {
      ...
      for (let c = 0; c < k; c++) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          for (let j = 0; j < p; j++)
            newMean[j] =
              (newMean[j] ?? 0) + r * ((X[i] as Float64Array)[j] ?? 0);
        }
        ...
        for (let i = 0; i < n; i++) {
          const r = (resp[i] as Float64Array)[c] ?? 0;
          for (let j = 0; j < p; j++)
            v += r * ((X[i] as Float64Array)[j] ?? 0 - (newMean[j] ?? 0)) ** 2;
        }
    ...
    this.means_ = means;
    this.covariances_ = means.map((_, c) => [
      new Float64Array(p).fill(variances[c] ?? 1),
    ]);
    return this;
  }
  ...
  predictProba(X: Float64Array[]): Float64Array[] {
    if (!this.weights_ || !this.means_)
      throw new NotFittedError("GaussianMixture is not fitted.");
    const variances = (this.covariances_ as Float64Array[][]).map(
      (c) => (c[0] as Float64Array)[0] ?? 1,
    );
