...
export class LabelSpreadingFull {
  ...
  fit(X: Float64Array[], y: Int32Array): this {
    ...
    for (let iter = 0; iter < this.maxIter; iter++) {
      ...
      for (let i = 0; i < nSamples; i++) {
        for (let c = 0; c < nClasses; c++) {
          FNew[i]![c] =
            this.alpha * (FNew[i]![c] ?? 0) +
            (1 - this.alpha) * (Y0[i]?.[c] ?? 0);
        }
        // Normalize rows
        const rowSum = FNew[i]!.reduce((s, v) => s + v, 0) || 1;
        for (let c = 0; c < nClasses; c++)
          FNew[i]![c] = (FNew[i]![c] ?? 0) / rowSum;
      }
  ...
  predict(X: Float64Array[]): Int32Array {
    if (!this.X_ || !this.labelDistributions_ || !this.classes_)
      throw new NotFittedError("LabelSpreadingFull");
    const nClasses = this.classes_.length;
  ...
  predictProba(X: Float64Array[]): Float64Array[] {
    if (!this.X_ || !this.labelDistributions_)
      throw new NotFittedError("LabelSpreadingFull");
    const nTrain = this.X_.length;
    const nFeatures = this.X_[0]?.length ?? 0;
