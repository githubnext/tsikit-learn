...
export class SelectFdr {
  ...
  fit(X: Float64Array[], y: Int32Array): this {
    ...
    const sortedIdx = Array.from({ length: nFeatures }, (_, i) => i).sort(
      (a, b) => (pValues[a] ?? 1) - (pValues[b] ?? 1),
    );
    ...
    this.supportMask_ = Array.from(
      { length: nFeatures },
      (_, j) => (pValues[j] ?? 1) <= threshold,
    );
    return this;
  }
  ...
  transform(X: Float64Array[]): Float64Array[] {
    ...
    return X.map((x) => {
      ...
      for (let j = 0; j < x.length; j++)
        if (this.supportMask_![j]) result.push(x[j] ?? 0);
...
export class SelectFwe {
  ...
  fit(X: Float64Array[], y: Int32Array): this {
    ...
    this.supportMask_ = Array.from(
      { length: nFeatures },
      (_, j) => (pValues[j] ?? 1) <= bonferroniAlpha,
    );
    return this;
  }
