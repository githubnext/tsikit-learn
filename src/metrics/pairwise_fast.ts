...
export function wassersteinDistance(u: Float64Array, v: Float64Array): number {
  ...
  let d = 0;
  for (let i = 0; i < n; i++)
    d += Math.abs((sorted_u[i] ?? 0) - (sorted_v[i] ?? 0));
...
export function jensenShannonDivergence(
  ...
  for (let i = 0; i < n; i++)
    m[i] =
      ((p[i] ?? 0) / Math.max(sumP, 1e-10) +
        (q[i] ?? 0) / Math.max(sumQ, 1e-10)) /
      2;
  let jsd = 0;
  for (let i = 0; i < n; i++) {
...
export function hausdorffDistance(
  ...
  const d1 = Math.max(
    ...X.map((x) => Math.min(...Y.map((y) => _euclidean(x, y)))),
  );
  const d2 = Math.max(
    ...Y.map((y) => Math.min(...X.map((x) => _euclidean(x, y)))),
  );
...
export function modifiedHausdorffDistance(
  ...
  const m = Y.length;
  const d1 =
    X.reduce((s, x) => s + Math.min(...Y.map((y) => _euclidean(x, y))), 0) / n;
  const d2 =
    Y.reduce((s, y) => s + Math.min(...X.map((x) => _euclidean(x, y))), 0) / m;
  return Math.max(d1, d2);
}
...
export function pairwiseWasserstein(
  X: Float64Array[],
  Y: Float64Array[],
): Float64Array[] {
  return X.map(
    (x) => new Float64Array(Y.map((y) => wassersteinDistance(x, y))),
  );
}

export function pairwiseJSD(
  X: Float64Array[],
  Y: Float64Array[],
): Float64Array[] {
  return X.map(
    (x) => new Float64Array(Y.map((y) => jensenShannonDivergence(x, y))),
  );
}
...
export function energyDistance(X: Float64Array[], Y: Float64Array[]): number {
  ...
  for (let i = 0; i < n; i++)
    for (let j = 0; j < n; j++) eXX += _euclidean(X[i]!, X[j]!);
  for (let i = 0; i < m; i++)
    for (let j = 0; j < m; j++) eYY += _euclidean(Y[i]!, Y[j]!);
...
export function bhattacharyyaDistance(
  ...
  for (let i = 0; i < n; i++)
    coef += Math.sqrt(
      (((p[i] ?? 0) / Math.max(sumP, 1e-10)) * (q[i] ?? 0)) /
        Math.max(sumQ, 1e-10),
    );
