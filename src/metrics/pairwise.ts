...
export function manhattanDistances(
  X: Float64Array[],
  Y?: Float64Array[],
): Float64Array[] {
  const A = Y ?? X;
...
export function cosineDistances(
  X: Float64Array[],
  Y?: Float64Array[],
): Float64Array[] {
    ...
    case "chebyshev": {
      ...
      return Array.from({ length: n }, (_, i) => {
        ...
        for (let j = 0; j < m; j++) {
          ...
          let d = 0;
          for (let k = 0; k < xi.length; k++)
            d = Math.max(d, Math.abs((xi[k] ?? 0) - (aj[k] ?? 0)));
          row[j] = d;
        }
        return row;
      });
...
export function rbfKernelMatrix(
  ...
  return D.map((row) =>
    Float64Array.from(row.map((d) => Math.exp(-g * d ** 2))),
  );
}
...
export function polynomialKernel(
  ...
  return lin.map((row) =>
    Float64Array.from(row.map((v) => (g * v + coef0) ** degree)),
  );
}
