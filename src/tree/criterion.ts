...
export function entropyImpurity(
  classCounts: Int32Array,
  total: number,
): number {
  if (total === 0) return 0;
  let h = 0;
...
export class BestSplitter {
  findBestSplit(
    ...
    for (const fi of features) {
      ...
      for (let ti = 0; ti < thresholds.length - 1; ti++) {
        ...
        const leftIdx = values
          .map((v, i) => (v <= t ? i : -1))
          .filter((i) => i >= 0);
        const rightIdx = values
          .map((v, i) => (v > t ? i : -1))
          .filter((i) => i >= 0);
        ...
        if (bestSplit === null || improvement > bestSplit.impurityImprovement) {
          bestSplit = {
            ...
            leftSize: leftIdx.length,
            rightSize: rightIdx.length,
          };
        }
      }
    }
  ...
  private _subset(
    ...
    if (y instanceof Int32Array)
      return new Int32Array(indices.map((i) => y[i] ?? 0));
  ...
  private _impurity(
    ...
    if (criterion === "mse")
      return mseCriterion(y instanceof Float64Array ? y : new Float64Array(y));
    if (criterion === "mae")
      return maeCriterion(y instanceof Float64Array ? y : new Float64Array(y));
...
export class RandomSplitter {
  constructor(
    private readonly maxFeatures: number | "sqrt" | "log2" | "auto" = "sqrt",
  ...
  findBestSplit(
    ...
    const k =
      this.maxFeatures === "sqrt"
        ? Math.ceil(Math.sqrt(nF))
        : this.maxFeatures === "log2"
          ? Math.ceil(Math.log2(Math.max(nF, 2)))
...
export class ExtraTreeSplitter {
  ...
  findRandomSplit(
    ...
    const yLArr =
      y instanceof Int32Array ? new Int32Array(yLeft) : new Float64Array(yLeft);
    const yRArr =
      y instanceof Int32Array
        ? new Int32Array(yRight)
        : new Float64Array(yRight);
    const n = y.length;
    const improvement =
      parentImpurity -
      (leftIdx.length / n) * this._impurity(yLArr, criterion) -
      (rightIdx.length / n) * this._impurity(yRArr, criterion);
    return {
      featureIndex: fi,
      threshold: t,
      impurityImprovement: improvement,
