...
export function selu(z: Float64Array): Float64Array {
  ...
  const alpha = 1.6732632423543774;
  const result = new Float64Array(z.length);
  for (let i = 0; i < z.length; i++) {
    const zi = z[i] ?? 0;
...
export function softplus(z: Float64Array): Float64Array {
  const result = new Float64Array(z.length);
  for (let i = 0; i < z.length; i++)
    result[i] = Math.log(1 + Math.exp(z[i] ?? 0));
  return result;
}
...
export function getActivationDerivative(
  name: ActivationName,
): ((z: Float64Array) => Float64Array) | null {
  switch (name) {
    case "sigmoid":
